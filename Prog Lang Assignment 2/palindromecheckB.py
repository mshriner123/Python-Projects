...
#It converts the input to lowercase and removes all whitespace, all ., all ;, and all , before checking
...
def is_palindrome(user_input): 
    ...
    #group 3: any character 1 or more times at the end of the user input
    
    palindrome = True

    user_input = user_input.casefold() # converts the user input to lower case

    user_input = user_input.replace(" ","")#removes all whitespace from the user input

    user_input = user_input.replace(".","")#removes all . from the user input
    ...
    while len(user_input) > 1 and palindrome:
        ...
        first_char = m.group(1)#first_char = the first character in the user input
        #if user input = "racecar" then first_char = "r"
        
        middle = m.group(2)#middle = the character(s) (if there are any) between the first character and the last in the user input
        #if user input = "racecar" then middle = "aceca"
        
        last_char = m.group(3)#last_char = the last character in the user input
        #if user input = "racecar" then last_char = "r"

        if first_char != last_char:
            palindrome = False

        user_input = middle #the user input is now everything between the first and last characters, excluding the first and last characters

    return palindrome #returns after the while loop
...
def get_user_input():
    ...
    while True: #loops until the user enters -1
        ...
        #if here, user input does not equal -1
